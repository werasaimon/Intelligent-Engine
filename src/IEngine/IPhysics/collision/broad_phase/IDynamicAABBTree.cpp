...
#include "../../common/memory/IStack.h"
...
u32 IDynamicAABBTree::allocateNode()
{
...
i32 IDynamicAABBTree::addObjectInternal(const IAABB& aabb)
{
    ...
    const IVector3 gap(mExtraAABBGap, mExtraAABBGap, mExtraAABBGap);
...
bool IDynamicAABBTree::updateObject(i32 nodeID, const IAABB& newAABB, const IVector3& displacement, bool forceReinsert)
{
...
void IDynamicAABBTree::insertLeafNode(i32 nodeID)
{
    ...
    IAABB newNodeAABB = mNodes[nodeID].aabb;
...
void IDynamicAABBTree::reportAllShapesOverlappingWithAABB(const IAABB& aabb, IDynamicAABBTreeOverlapCallback& callback) const
{
...
void IDynamicAABBTree::raycast(const IRay& ray, IDynamicAABBTreeRaycastCallback &callback) const
{
    ...
    while (stack.Count() > 0)
    {
        ...
        IRay rayTemp(ray.point1, ray.point2, maxFraction);
