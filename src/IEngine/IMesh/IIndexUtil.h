...
void apply_map( IMath::IVector3D<T> & v, Func mapper )
{
    v[0] = mapper[v[0]];
    v[1] = mapper[v[1]];
    v[2] = mapper[v[2]];
...
// return v[i] = mapper(v[i])
template<typename T, typename Func>
IMath::IVector3D<T> apply_map( const IMath::IVector3D<T> & vIn, Func mapper )
{
    return IMath::IVector3D<T>( mapper[vIn[0]], mapper[vIn[1]], mapper[vIn[2]] );
}
