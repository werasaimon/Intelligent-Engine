...
namespace IPhysics
{
	...
	namespace quickhull
	{
        ...
        template<class T> using Vector3 = IMath::IVector3D<T>;
...
//		};
		...
		template <typename T>
		SIMD_INLINE std::ostream& operator<<(std::ostream& os, const Vector3<T>& vec)
		{
		...
		template <typename T>
		SIMD_INLINE Vector3<T> operator*(T c, const Vector3<T>& v)
		{
