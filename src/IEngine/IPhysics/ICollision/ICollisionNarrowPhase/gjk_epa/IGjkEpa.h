...
struct	IGjkEpa
{
    ...
    static bool Distance(const ConvexTemplate& a,
                         ...
                         const IVector3& guess, IGjkEpa::sResults& results);
    ...
    static i32	 ComputeGjkDistance(const ConvexTemplate& a,
                                    const ConvexTemplate& b,
...
struct	MinkowskiDiff
{
    ...
    SIMD_INLINE IVector3  Support0( const IVector3& dir ) const
    {
        IVector3 p  = mWorld0 * (mConvexAPtr->GetLocalSupportPointWithMargin( mWorld0.GetBasis().GetTranspose() * dir));
        //IVector3 p  = (mConvexAPtr->GetWorldSupportPointWithMargin(dir));
        return  p;
    }

    SIMD_INLINE IVector3	 Support1( const IVector3& dir ) const
    {
        IVector3 p  = mWorld1 * (mConvexBPtr->GetLocalSupportPointWithMargin( mWorld1.GetBasis().GetTranspose() * dir));
       //IVector3 p  = (mConvexBPtr->GetWorldSupportPointWithMargin(dir));
        return p;
    }

    SIMD_INLINE  IVector3	Support(const IVector3& d) const
    {
        return(Support0(d)-Support1(-d));
    }

    SIMD_INLINE  IVector3	Support(const IVector3& d,U index) const
    {
        if(index)
            return(Support1(d));
        else
...
struct	GJK
{
    ...
    eGjkStatus	Evaluate(const MinkowskiDiff<ConvexTemplate>& shapearg,const IVector3 &guess)
    {
        U			iterations=0;
        scalar	sqdist=0;
        scalar	alpha=0;
        ...
        appendvertice(mSimplices[0],sqrl>0?-mRay:IVector3	(1,0,0));
        mSimplices[0].p[0]	=	1;
        ...
        do	{
            ...
            const IVector3	&	w=cs.c[cs.rank-1]->w;
            bool				found=false;
            for(U i=0;i<4;++i)
            {
        ...
        bool	EncloseOrigin()
        {
            ...
            case	2:
            {
                const IVector3	d=mSimplex->c[1]->w-mSimplex->c[0]->w;
                for(U i=0;i<3;++i)
                {
                    IVector3		axis=IVector3	(0,0,0);
                    axis[i]=1;
                    const IVector3	p=Cross(d,axis);
                    if(p.LengthSquare()>0)
                    {
            ...
            case	3:
            {
                const IVector3	n=Cross(mSimplex->c[1]->w-mSimplex->c[0]->w,
        ...
        /* Internals	*/
        void	getsupport(const IVector3	& d,sSV& sv) const
        {
            sv.d	=	d/d.Length();
        ...
        void				appendvertice(sSimplex& simplex,const IVector3	& v)
        {
            simplex.p[simplex.rank]=0;
            simplex.c[simplex.rank]=mFree[--mNfree];
            getsupport(v,*simplex.c[simplex.rank++]);
        }
        static scalar	det(const IVector3	& a,const IVector3	& b,const IVector3	& c)
        {
            return( a.y*b.z*c.x+a.z*b.x*c.y-
                    a.x*b.z*c.y-a.y*b.x*c.z+
                    a.x*b.y*c.z-a.z*b.y*c.x);
        }
        static scalar	projectorigin(	const IVector3	& a,
                                        const IVector3	& b,
                                         scalar* w,U& m)
        {
            const IVector3	d=b-a;
                                         ...
                                         scalar* w,U& m)
        {
            ...
            const IVector3*	    vt[]={&a,&b,&c};
            const IVector3		dl[]={a-b,b-c,c-a};
            const IVector3		n=Cross(dl[0],dl[1]);
        ...
        {
            static const U		imd3[]={1,2,0};
            const IVector3	*	vt[]={&a,&b,&c,&d};
            const IVector3		dl[]={a-d,b-d,c-d};
    ...
    struct	EPA
    {
        ...
        eEpaStatus	Evaluate(GJK<ConvexTemplate>& gjk,const IVector3& guess)
        {
            ...
            if((simplex.rank>1)&&gjk.EncloseOrigin())
            {
                ...
                if(mHull.count==4)
                {
                    ...
                    const IVector3	projection=outer.n*outer.d;
                    mNormal	        =	outer.n;
                    mDepth		    =	outer.d;
                    mResult.rank	=	3;
        ...
        {
            const IVector3 ba = b->w - a->w;
            const IVector3 n_ab = Cross(ba, face->n); // Outward facing edge normal direction, on triangle plane
                            ...
                            MinkowskiDiff<ConvexTemplate>& shape)
    {
        ...
        results.mWitnesses[1]	=	IVector3(0,0,0);
