...
namespace IPhysics
{
	...
	namespace quickhull
	{
		...
		class QuickHull
		{
			...
			SIMD_INLINE std::unique_ptr<std::vector<IndexType>> getIndexVectorFromPool();
			SIMD_INLINE void reclaimToIndexVectorPool(std::unique_ptr<std::vector<IndexType>>& ptr);
			...
			SIMD_INLINE bool addPointToFace(typename MeshBuilder<FloatType>::Face& f, IndexType pointIndex);
