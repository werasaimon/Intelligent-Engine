...
scalar ISliderJoint::GetTranslation() const
{
    ...
    const IVector3 anchorBody1 = x1 + q1.GetRotMatrix() * mLocalAnchorPointBody1;
    const IVector3 anchorBody2 = x2 + q2.GetRotMatrix() * mLocalAnchorPointBody2;

    // Compute the vector u (difference between anchor points)
    const IVector3 u = anchorBody2 - anchorBody1;

    // Compute the slider axis in world-space
    IVector3 sliderAxisWorld = q1.GetRotMatrix() * mSliderAxisBody1;
