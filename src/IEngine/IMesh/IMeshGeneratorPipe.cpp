...
void GenerateMesh(const PipeDescriptor& desc, IMeshModel& mesh)
{
    ...
    IVector3 coord, normal, coordAlt;
    IVector2 texCoord;

    auto angle = float(0);

    const IVector2 radii[2] = { desc.outerRadius, desc.innerRadius };
    ...
    for (std::size_t i = 0; i < 2; ++i)
    {
        ...
        const auto invRadius = IVector2(1) / (desc.outerRadius * float(2.0));
        ...
        for (std::uint32_t u = 0; u <= segsHorz; ++u)
        {
            ...
            for (std::uint32_t v = 0; v <= segsCov[i]; ++v)
            {
                ...
                auto texCoordA = IVector2(float(0.5)) + IVector2(coordAlt.x, coordAlt.z) * invRadius;
                auto texCoordB = IVector2(float(0.5)) + IVector2(coord.x, coord.z) * invRadius;
