...
#include "../IMemory/dvector.h"
#include "../IMemory/refcount_vector.h"
#include "../IMemory/small_list_set.h"

#include "IIndexUtil.h"
#include "IIintersectionUtil.h"
...
struct VertexInfo
{
    ...
    VertexInfo()
    {
        this->v = IVector3::ZERO; n = c = IVector3::ZERO; uv = IVector2::ZERO;
        bHaveN = bHaveC = bHaveUV = false;
    }

    VertexInfo(const IVector3 & v)
    {
        this->v = v; n = c = IVector3::ZERO; uv = IVector2::ZERO;
        bHaveN = bHaveC = bHaveUV = false;
    }

    VertexInfo(const IVector3 & v, const IVector3 & n)
    {
        this->v = v; this->n = n; c = IVector3::ZERO; uv = IVector2::ZERO;
        bHaveN = true; bHaveC = bHaveUV = false;
    }

    VertexInfo(const IVector3 & v, const IVector3 & n, const IVector3 & c)
    {
        this->v = v; this->n = n; this->c = c; uv = IVector2::ZERO;
        bHaveN = bHaveC = true; bHaveUV = false;
    }

    VertexInfo(const IVector3 & v, const IVector3 & n, const IVector3 & c, const IVector2 & uv)
    {
...
class IMeshModel : public IObjectTransform , public IObjectAxisAlignedBox
{
...
public:
    ...
    static IVector3 InvalidVertex()   { return IVector3(max_float, 0, 0); }
...
//    IMeshModel() = default;
//    IMeshModel(const IMeshModel&) = default;


    IMeshModel(bool bWantNormals = true, bool bWantColors = false, bool bWantUVs = false, bool bWantTriGroups = false)
    {
    ...
    IMeshModel(MeshComponents flags) :
        IMeshModel( ((int)flags & (int)MeshComponents::VertexNormals) != 0,  ((int)flags & (int)MeshComponents::VertexColors) != 0,
    ...
    // normals/colors/uvs will only be copied if they exist
     IMeshModel(const IMeshModel & copy, bool bCompact = false, bool bWantNormals = true, bool bWantColors = true, bool bWantUVs = true)
     {
         if (bCompact)
         {
     ...
     IMeshModel(const IMeshModel& copy, bool bCompact, MeshComponents flags) :
         IMeshModel(copy, bCompact,
     ...
     void ClearFullData()
     {
         mAxisAlignedBox = IAxisAlignedBox3D::Zero;
...
public:
    ...
    IAxisAlignedBox3D BoundingAxisAlignedBoxTransform() const
    {
        return mAxisAlignedBox.GetAxisAlignedBoxTransform(mTransformMatrix);
    }
    ...
    {
        int random_index_ID = rand()%VertexCount();
        this->RemoveVertex(random_index_ID);
    }
...
public:
    ...
    CompactInfo CompactCopy(const IMeshModel& copy, bool bNormals = true, bool bColors = true, bool bUVs = true)
    {
    ...
    void Copy(const IMeshModel& copy, bool bNormals = true, bool bColors = true, bool bUVs = true)
    {
...
public:
    ...
    IVector3 GetVertex(int vID) const
    {
        debug_check_is_vertex(vID);
        int i = 3 * vID;
        return IVector3( mVertices [i], mVertices[i + 1], mVertices[i + 2]);
    }


    IVector3 GetVertexf(int vID) const
    {
        debug_check_is_vertex(vID);
        int i = 3 * vID;
        return IVector3((float)mVertices[i], (float)mVertices[i + 1], (float)mVertices[i + 2]);
    }

    void SetVertex(int vID, const IVector3 & vNewPos)
    {
    ...
    IVector3 GetVertexNormal(int vID) const
    {
        ...
        debug_check_is_vertex(vID);
        int i = 3 * vID;
        return IVector3(mNormals[i], mNormals[i + 1], mNormals[i + 2]);
    }

    void SetVertexNormal(int vID, IVector3 vNewNormal)
    {
        if ( HasVertexNormals() )
        {
            debug_check_is_vertex(vID);
    ...
    IVector3 GetVertexColor(int vID) const
    {
        if (HasVertexColors() == false)
            return IVector3::IDENTITY;
        debug_check_is_vertex(vID);
        int i = 3 * vID;
        return IVector3(mColors[i], mColors[i + 1], mColors[i + 2]);
    }

    void SetVertexColor(int vID, IVector3 vNewColor)
    {
        if ( HasVertexColors() )
        {
            debug_check_is_vertex(vID);
    ...
    IVector2 GetVertexUV(int vID) const
    {
        if (HasVertexUVs() == false)
            return IVector2::ZERO;
        debug_check_is_vertex(vID);
        int i = 2 * vID;
        return IVector2(mUV[i], mUV[i + 1]);
    }

    void SetVertexUV(int vID, IVector2 vNewUV)
    {
        if ( HasVertexUVs() )
        {
            debug_check_is_vertex(vID);
    ...
    bool GetVertex(int vID, VertexInfo& vinfo, bool bWantNormals, bool bWantColors, bool bWantUVs) const
    {
        if (vertices_refcount.isValid(vID) == false)
            return false;
        vinfo.v = IVector3(mVertices[3 * vID], mVertices[3 * vID + 1], mVertices[3 * vID + 2]);
        vinfo.bHaveN = vinfo.bHaveUV = vinfo.bHaveC = false;
        if (HasVertexNormals() && bWantNormals)
        {
            vinfo.bHaveN = true;
            vinfo.n = IVector3(mNormals[3 * vID], mNormals[3 * vID + 1], mNormals[3 * vID + 2]);
        }

        if (HasVertexColors() && bWantColors)
        {
            vinfo.bHaveC = true;
            vinfo.c = IVector3(mColors[3 * vID], mColors[3 * vID + 1], mColors[3 * vID + 2]);
        }

        if (HasVertexUVs() && bWantUVs)
        {
            vinfo.bHaveUV = true;
            vinfo.uv = IVector2(mUV[2 * vID], mUV[2 * vID + 1]);
        }
...
//        int vi = 3 * vID;
//        IVector3 v(vertices[vi], vertices[vi + 1], vertices[vi + 2]);
//        IVector3 normal(normals[vi], normals[vi + 1], normals[vi + 2]);
//        int eid = mVertexEdges.First(vID);
//        int ovi = 3 * edge_other_v(eid, vID);
//        IVector3 ov(vertices[ovi], vertices[ovi + 1], vertices[ovi + 2]);
//        IVector3 edge = (ov - v);
//        edge.normalize();

//        IVector3 other = normal.cross(edge);
...
//    }
    ...
    void GetTriVertices(int tID, IVector3 & v0, IVector3 & v1, IVector3 & v2) const
    {
    ...
    IVector3 GetTriVertex(int tid, int j) const
    {
        int a = mTriangles[3 * tid + j];
        return IVector3(mVertices[3 * a], mVertices[3 * a + 1], mVertices[3 * a + 2]);
    }

    IVector3 GetTriBaryPoint(int tID, float bary0, float bary1, float bary2) const
    {
        int ai = 3 * mTriangles[3 * tID],
    ...
    IVector3 GetTriNormal(int tID) const
    {
        IVector3 v0, v1, v2;
        GetTriVertices(tID, v0, v1, v2);
        return IVector3::triNormal(v0, v1, v2);
    }
    ...
    /// </summary>
    void GetTriInfo(int tID, IVector3 & normal, float & fArea, IVector3 & vCentroid) const
    {
        IVector3 v0, v1, v2;
        GetTriVertices(tID, v0, v1, v2);
        vCentroid = (1.0 / 3.0) * (v0 + v1 + v2);
        fArea = IVector3::Area(v0, v1, v2);
        normal = IVector3::triNormal(v0, v1, v2);
    ...
    /// </summary>
    IVector3 GetTriBaryNormal(int tID, float bary0, float bary1, float bary2)
    {
        int ai = 3 * mTriangles[3 * tID],
            bi = 3 * mTriangles[3 * tID + 1],
            ci = 3 * mTriangles[3 * tID + 2];
    ...
    /// </summary>
    IVector3 GetTriCentroid(int tID) const
    {
        int ai = 3 * mTriangles[3 * tID],
            bi = 3 * mTriangles[3 * tID + 1],
            ci = 3 * mTriangles[3 * tID + 2];
...
//        }
//        return AxisAlignedBox3d(Wml::IVector3(minx, miny, minz), Wml::IVector3(maxx, maxy, maxz));
//    }
...
//        IVector3 v1(vertices[a], vertices[a + 1], vertices[a + 2]);
//        IVector3 v2(vertices[b], vertices[b + 1], vertices[b + 2]);
//        IVector3 v3(vertices[c], vertices[c + 1], vertices[c + 2]);

//        IVector3 edge1 = v2 - v1;  edge1.normalize();
//        IVector3 edge2 = v3 - v2;  edge2.normalize();
//        IVector3 normal = edge1.cross(edge2); normal.normalize();

//        IVector3 other = normal.cross(edge1);

//        IVector3 center = (v1 + v2 + v3) / 3;
...
//    }
    ...
    /// </summary>
    float GetTriSolidAngle(int tID, const IVector3 & p) const
    {
        int ti = 3 * tID;
        int ta = 3 * mTriangles[ti];
        IVector3 a(mVertices[ta] - p.x, mVertices[ta + 1] - p.y, mVertices[ta + 2] - p.z);
        int tb = 3 * mTriangles[ti + 1];
        IVector3 b(mVertices[tb] - p.x, mVertices[tb + 1] - p.y, mVertices[tb + 2] - p.z);
        int tc = 3 * mTriangles[ti + 2];
        IVector3 c(mVertices[tc] - p.x, mVertices[tc + 1] - p.y, mVertices[tc + 2] - p.z);
...
//        IVector3 a(mVertices[ta], mVertices[ta + 1], mVertices[ta + 2]);
//        int tb = 3 * triangles[ti + 1];
//        IVector3 b(mVertices[tb], mVertices[tb + 1], mVertices[tb + 2]);
//        int tc = 3 * triangles[ti + 2];
//        IVector3 c(mVertices[tc], mVertices[tc + 1], mVertices[tc + 2]);
//        if ( i == 0 )
...
//    }
    ...
    bool GetEdgeV(int eID, IVector3 & a, IVector3 & b) const
    {
        debug_check_is_edge(eID);
    ...
    IVector3 GetEdgeNormal(int eID) const
    {
        if (edges_refcount.isValid(eID))
        {
            int ei = 4 * eID;
            IVector3 n = GetTriNormal(mEdges[ei + 2]);
            if (mEdges[ei + 3] != InvalidID)
            {
    ...
    IVector3 GetEdgePoint(int eID, float t) const
    {
        if (edges_refcount.isValid(eID))
        {
            int ei = 4 * eID;
    ...
    /// </summary>
    int AppendVertex(const IVector3 & v)
    {
        VertexInfo vinfo(v);
    ...
    int AppendVertex(const VertexInfo & info)
    {
        ...
        IObjectAxisAlignedBox::InsertCalculate(info.v);
        ...
        if ( HasVertexNormals() )
        {
            IVector3 n = (info.bHaveN) ? info.n : IVector3::Y;
        ...
        if ( HasVertexColors() )
        {
            IVector3 c = (info.bHaveC) ? info.c : IVector3::IDENTITY;
        ...
        if ( HasVertexUVs() )
        {
            IVector2 u = (info.bHaveUV) ? info.uv : IVector2::ZERO;
            int j = 2*vid;
    ...
    /// </summary>
    int AppendVertex(IMeshModel from, int fromVID)
    {
        int bi = 3 * fromVID;
    ...
    bool IntersectRay(const IRay &ray) const
    {

        IRay local_ray(  ray.Origin    * mTransformMatrix.GetInverse(),
                         ray.Direction * mTransformMatrix.GetRotMatrix().GetInverse());

        for(std::size_t i = 0; i < mTriangles.size(); i+=3)
        {
            IVector3 a = GetVertex(mTriangles[i+0]);// * mTransformMatrix;
            IVector3 b = GetVertex(mTriangles[i+1]);// * mTransformMatrix;
            IVector3 c = GetVertex(mTriangles[i+2]);// * mTransformMatrix;

            if(IRayIntersectionToTriangle(a,b,c,local_ray.Origin,local_ray.Direction))
            {
                return true;
            }
        }
    ...
    void EnableVertexNormals(IVector3 initial_normal)
    {
        if (HasVertexNormals())
            return;
        mNormals = IMemory::dvector<float>();
    ...
    void EnableVertexColors(IVector3 initial_color)
    {
        if (HasVertexColors())
            return;
        mColors = IMemory::dvector<float>();
    ...
    void EnableVertexUVs(IVector2 initial_uv)
    {
        if (HasVertexUVs())
            return;
        mUV = IMemory::dvector<float>();
    ...
    value_iteration<IVector3> Vertices() const
    {
        return vertices_refcount.mapped_indices<IVector3>( [=](int vid)
        {
            int i = 3 * vid;
            return IVector3(mVertices[i], mVertices[i + 1], mVertices[i + 2]);
        });
    ...
    /// </summary>
    void VtxOneRingCentroid(int vID, IVector3 & centroid) const
    {
        centroid = IVector3::ZERO;
        if (vertices_refcount.isValid(vID))
        {
            int n = 0;
            for (int eid : mVertexEdges.values(vID))
...
//               if (z < minz) minz = z; else if (z > maxz) maxz = z;
//           }
//           return AxisAlignedBox3d(Wml::IVector3(minx, miny,minz), Wml::Vector3d(maxx, maxy, maxz));
//       }
...
//       }
       ...
       /// </summary>
       float WindingNumber(IVector3 v) const
       {
           float sum = 0;
           for ( int tid : TriangleIndices() )
       ...
       MeshResult SplitEdge(int eab, EdgeSplitInfo & split, float split_t = 0.5)
       {
           ...
           if ( IsBoundaryEdge(eab) )
           {
               ...
               // create vertex
               IVector3 vNew = Lerp(GetVertex(a), GetVertex(b), split_t);
...
//       }
       ...
       struct MergeEdgesInfo
       {
           ...
           IVector2i vKept;
           IVector2i vRemoved;           // either may be InvalidID if it was same as vKept

           IVector2i eRemovedExtra;      // bonus collapsed edge, or InvalidID
           IVector2i eKeptExtra;			// edge paired w/ eRemovedExtra
       };
       ...
       MeshResult MergeEdges(int eKeep, int eDiscard, MergeEdgesInfo & merge_info)
       {
           ...
           IVector3 Va = GetVertex(a), Vb = GetVertex(b), Vc = GetVertex(c), Vd = GetVertex(d);
           ...
           //IVector3 otherv = GetVertex(tcd_otherv);
           //IVector3 Ncd = MathUtil.FastNormalDirection(GetVertex(c), GetVertex(d), otherv);
           //IVector3 Nab = MathUtil.FastNormalDirection(GetVertex(a), GetVertex(b), otherv);
           //if (Ncd.Dot(Nab) < 0)
           //return MeshResult::Failed_SameOrientation;
           ...
           merge_info.eRemovedExtra = IVector2i(InvalidID, InvalidID);
       ...
       {
           return PokeTriangle(tid, IVector3::IDENTITY / 3.0, result);
       }

       virtual MeshResult PokeTriangle(int tid, const IVector3 & baryCoordinates, PokeTriangleInfo & result)
       {
...
//       /// </summary>
//       bool IsSameMesh(const IMeshModel & m2, bool bCheckConnectivity, bool bCheckEdgeIDs = false,
