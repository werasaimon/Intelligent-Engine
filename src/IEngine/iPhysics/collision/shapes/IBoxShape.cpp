...
#define BOX_MAX_PETURBERATION_ITERATIONS	      4
#define BOX_EPS_PETURBERATION_ANGLES_COFFICIENT   0.08
...
IBoxShape::IBoxShape(const IVector3 &extent)
    : IConvexShape(BOX, OBJECT_MARGIN) ,
      mExtent(extent - IVector3(OBJECT_MARGIN,OBJECT_MARGIN,OBJECT_MARGIN))
{
    ...
    mNbMaxPeturberationIteration = BOX_MAX_PETURBERATION_ITERATIONS; // maximum iteration  for Axis Peturberation
    mEpsilonPeturberation =  BOX_EPS_PETURBERATION_ANGLES_COFFICIENT;// epsilon for Peturberation
}
 ...
 * @param margin The collision margin (in meters) around the collision shape
 */
IBoxShape::IBoxShape(const IVector3& extent, scalar margin)
: IConvexShape(BOX, margin) ,
 ...
 */
void IBoxShape::computeLocalInertiaTensor(IMatrix3x3 &tensor, scalar mass) const
{
...
bool IBoxShape::raycast(const IRay& ray, IRaycastInfo& raycastInfo, IProxyShape* proxyShape) const
{
    ...
    IVector3 localHitPoint = ray.point1 + tMin * rayDirection;
