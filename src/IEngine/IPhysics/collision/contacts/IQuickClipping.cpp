...
bool IQuickClipping::InsidePolygonSAT(const IVector3 &vIntersection, const IVector3 *PolyVertices, u32 NbCount, const IQuickClipping::IClipPlane &plane)
{
    IVector3 vA, vB;
    IVector3 vNormalAxis;
    for (u32 i = 0; i < NbCount; i++)
    {
        vA = PolyVertices[i];
        ...
        if( (vIntersection - vB).dot(vNormalAxis) > 0.0 ) return false;
    }
